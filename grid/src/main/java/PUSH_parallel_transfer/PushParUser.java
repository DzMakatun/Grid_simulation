 ...
 */
public class PushParUser extends GridUser {
    ...
    // constructor
    public PushParUser(String name, double baud_rate, double delay, int MTU) throws Exception {
    ...
    private void initPlaner(){
		...
		for(NetworkLink link : PushParNetReader.planerLinks){
		    //solver.addLink(link);
		    allLinkFlows.add(
	...
	for(i = 0; i <  totalResource; i++){ 
		...
		gl.setUserID(myId_);
		//write("storageId: " + storageId + " resourceID[i] ");
		gl.setUsedLink(FlowManager.getLinkFlows(storageId, resourceID[i]));
		//write("used link: " + FlowManager.getLinkFlows(storageId, resourceID[i]).toString());
		sim_schedule(super.output, GridSimTags.SCHEDULE_NOW, GridSimTags.FLOW_SUBMIT,
