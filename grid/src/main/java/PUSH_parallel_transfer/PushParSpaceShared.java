 ...
 * @invariant $none
 */
class PushParSpaceShared extends AllocPolicy
{
     ...
     * @post $none
     */
    PushParSpaceShared(String resourceName, String entityName) throws Exception
    {
    ...
    public synchronized void gridletSubmit(Gridlet gl, boolean ack)
    {
        ...
        try{
            ...
            //write("Used link: " + dpGl.getUsedLink().toString());
    ...
    private void gridletFinish(ResGridlet rgl, int status)
    {
        ...
        rgl.finalizeGridlet();
        //write("gridlet finished: " + rgl.getGridletID());
        try {
            DPGridlet dpGl = (DPGridlet) rgl.getGridlet();
            dpGl.setUsedLink(FlowManager.getLinkFlows(this.resId_, storageId));
            //write("gridlet send: " + rgl.getGridletID() + " to " + dpGl.getUserID());
    	    sim_schedule(super.outputPort_, GridSimTags.SCHEDULE_NOW, GridSimTags.FLOW_SUBMIT,
		new IO_data(dpGl, dpGl.getGridletOutputSize(), dpGl.getUserID()) );
	...
	//write("Received event: " + ev.get_tag() );
	//extract event tag
	int tag = ev.get_tag();
	      	
	switch (tag){
	     case GridSimTags.FLOW_SUBMIT:
    ...
    private void processIncommingFlow(Sim_event ev) {
	DPGridlet gl = (DPGridlet) ev.get_data();
	//write("submitting gridlet: " + gl.getGridletID());
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        //BUGFIX (otherwise job times are not calculated)
        gl.setResourceParameter(this.resId_,this.resource_.getCostPerSec());
	this.gridletSubmit(gl, false);
	
    }

    private void write(String message){
	String indent = " ";
	System.out.println(GridSim.clock() + indent 
		+ this.myId_ + ":" + this.get_name() 
		+ "resource " + resName_ + ":" + this.resId_ + indent
		+ message);
