 ...
 */
public class NoNetworkUser extends GridUser {
    ...
    // constructor
    public NoNetworkUser(String name, double baud_rate, double delay, int MTU) throws Exception {
	...
	for(i = 0; i <  totalResource; i++){ 
		...
		sim_schedule(resourceID[i], GridSimTags.SCHEDULE_NOW, GridSimTags.GRIDLET_SUBMIT,
			gl);
	...
	for (i = 0; i < totalGridlet; i++){ //loop over received gridlets
		...
		gl.setUserID(myId_);
		//Send without network delay
		sim_schedule(resourceFromID, GridSimTags.SCHEDULE_NOW, GridSimTags.GRIDLET_SUBMIT,
			gl );
		sendTime[j]=  GridSim.clock(); //remember the time when the gridlet was submited
		j++;
		if (j == totalGridlet){
